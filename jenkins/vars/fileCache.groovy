...
// FIXME: Need to avoid concurrent access by multiple runing jobs.
    ...
    tag="${name}/${item}"
    sum_file=${cache_path}/${name}/${item}.sum
    stamp_file=${cache_path}/${name}/${item}.stamp
    tmp_file=${cache_path}/${name}/${item}.tmp

    mkdir -p ${cache_path}/${name}

    if [[ -f ${item} ]]; then
        cp -f ${item} \${tmp_file}
        dest=${cache_path}/${name}/${item}
    elif  [[ -d ${item} ]]; then
        sudo tar -cf \${tmp_file} ${item}
        dest=${cache_path}/${name}/${item}.tar
    else
    ...
    if [[ -f \${stamp_file} && -f \${sum_file} \
        ...
        echo "cache-put: Found '\${tag}' in ${cache_path}." >&2
        rm -f \${tmp_file}
        exit 0
    fi
    ...
    echo "tag:\${tag}" >> \${stamp_file}
    echo "date:\$(date)" >> \${stamp_file}
    echo "md5sum:\${sum}" >> \${stamp_file}
    echo "${stamp_info}" >> \${stamp_file}

    echo "cache-put: Wrote '\${tag}' to ${cache_path}." >&2
    exit 0
""")
}

boolean get(String cache_path, String name, String item, boolean use_flag) {
    if (!use_flag) {
    ...
    dest=${item}
    if [[ -f ${cache_path}/${name}/${item} ]]; then
        src=${cache_path}/${name}/${item}
    elif  [[  -f ${cache_path}/${name}/${item}.tar ]]; then
        have_tar=1
        src=${cache_path}/${name}/${item}.tar
    else
        echo "cache-get: '\${tag}' not found in ${cache_path}." >&2
        exit 1
    fi
    ...
    echo "cache-get: Using '\${tag}' from ${cache_path}." >&2
